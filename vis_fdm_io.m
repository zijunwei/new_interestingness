...
VideoSavePath=fullfile(datasetup.gazeDatasetDir,'visualization_IO');
...
for i=1:1:length(VideoEntropy_new)
    if length(VideoEntropy_new(i).entropy)>250
       continue; 
    end
    fprintf('---processing %s \t [ %04d | %04d], total %d frames\n',VideoEntropy_new(i).videoname,i,length(VideoEntropy_new),length(VideoEntropy_new(i).entropy));
    videoObj=VideoReader(fullfile(datasetup.videoDir,VideoEntropy_new(i).videoname));
    FrameCount=1;
    
    videoWObj=VideoWriter(fullfile(VideoSavePath,VideoEntropy_new(i).videoname));
    open(videoWObj);
    
    
    while hasFrame(videoObj)
        ml_progressBar(FrameCount,length(VideoEntropy_new(i).entropy));
        videoFrame=readFrame(videoObj);
        
        subplot(2,1,1)
        imshow(videoFrame);
        idx=z_structfind(VideoInformation,'videoname',VideoEntropy_new(i).videoname);
        hold on;
        ...
        v_entropy=VideoEntropy_new(i).entropy;
